...
2. 不需要增加子类，来拓展对象的功能。而且装饰类和被装饰类 可以独立发展，不会相互耦合。
### 拓展（了解）
...
### 个人理解&注意事项
定义全局instance变量来实现单例模式的关键，每次创建对象时都会判断instance是否为None，如果为None进行初始化，如果不为None直接返回现有的instance。

framework2.py需要理解自定义metaclass创建对象的过程，理清__init__和__call__的调用顺序。

### 拓展
1. new、init和call的用法

    （1）__new__负责对象的创建，而__init__负责对象的初始化；

    （2）__new__是一个类方法，而__init__和__call__是一个对象方法；

    （3）__call__声明这个类的对象是可调用的（callable）。

   + new是构造函数，需要返回一个实例
   + init是初始化函数，对new实例化的对象进行初始化，不允许有返回值
   + init方法中除了self定义的参数，其他参数都必须与new方法中除cls参数外的参数保持一致。
     + 一个对象从创建到被调用的大致过程：
       + __new__方法是通过类名进行实例化对象时自动调用的；
       + __init__是在每一次实例化对象之后调用的；
       + __new__方法创建一个实例之后返回这个实例对象，并将其传递给__init__方法的 self参数。
   + __call__方法
     + 内置函数callable，用来指示某个对象是否为可调用的；
     + __call__的作用就是声明这个类的对象是可调用的。
2. metaclass
   1. 自定义metaclass
      ![](asserts/AppendixC_CustomMetaclass.png)

## 代理模式
为其他对象提供一种代理以控制对这个对象的访问。

代理的英文叫做Proxy或Surrogate。
### 作用
...
2. 代理下载（downloader.py）
### 模型说明
1. 模型优缺点
   + 优点
     1. 协调调用者和被调用者，降低系统的耦合度
     2. 既可以隐藏被代理对象的部分功能，又可以拓展功能和服务
   + 缺点 
     
     在调用者和被调用者之间增加了代理对象的处理
### 个人理解&注意事项
1. （简单情况下）由于代理对象帮助被代理对象完成服务，因此代理对象无需定义request方法，直接调用real_subject对象的request方法即可。
2. 有些对象由于某些原因（比如对象创建开销很大，或者某些操作需要安全控制，或者需要进程外的访问），而代理可以缓存先前计算过的结果，以避免重复计算。
3. 代理类可以在被代理类的基础上提供一些额外的功能，如：预处理和后处理。
